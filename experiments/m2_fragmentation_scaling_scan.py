...
def _run_size(
    ...
    for checkpoint_index, sweep in enumerate(checkpoints):
        ...
        if metrics["raw_target_uniqueness_violation_count"]:
            raise RuntimeError("distinct-target invariant violated during scan")
        row = {
            "N": size,
            "seed": seed,
            "worker_count": workers,
        ...
        print(
            ...
            f"duplicates={metrics['duplicate_slot_ratio']:.6f}",
            flush=True,
        )
